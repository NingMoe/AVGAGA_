...
class FFmpeg
{
	...
	*/
	 ...
	 *
	*/
	private $ffmpeg                =  '/root/bin/ffmpeg';// [linux] // 'ffmpeg';  // [windows]
	/**
	 *
	 */
	...
	*/
	...
	public function output( $output = null , $forceFormat = null )
	{
		...
		// ----------------------------------------------------------------------------------------------------
		// edit by AC  增加   -movflags +faststart  for 更改mp4格式為邊下載邊播放
		//  -cpu-used 2 -threads 4 -pass 2  => 使用2顆CPU  開4條執行緒下去跑,  pass 2次 確保畫面品質
		$this->command = $this->ffmpeg." ".join(' ',$options)." -movflags +faststart ".
		"  -cpu-used 2 -threads 4 -pass 1 ".$output; // . $this->STD; // -strict -2
	...
	public function ready( $append = null )
	{
		...
		if(empty( $this->command ))
		{
			...
			return exec( $this->command . $append );	// 針對linux 的調整
		}
	}
	/**
	 * @return        object        Return self
